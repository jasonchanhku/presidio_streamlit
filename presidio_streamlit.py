...
if st_model == "Azure AI Language":
    st_ta_key = os.getenv("TA_KEY", "")
    st_ta_endpoint = os.getenv("TA_ENDPOINT", default="")
...
def set_up_openai_synthesis():
    ...
    st_openai_model = st.sidebar.text_input(
        ...
        value=os.getenv("OPENAI_MODEL", default="eaom_gpt4_0613"),
...
with st.expander("[PLEASE READ] About this PII DLP layer demo by HKIRD", expanded=False):
    st.info(
        """
    Important Remarks:
    - This is at core a NER and pattern based model in the pipeline used to identify PII patterns based on a certain base model. Though not exhaustive, we can also specify or train it for patterns to look for.
    - Experiment with different off-the-shelf models models for PII de-identification.
    - Azure AI Language model supports multilingual 
    - There are different de-identification options, including redaction, masking, encryption and more.
    - Allow and deny lists can be configured accordingly.
    
    For more info, please contact HK_IRD_Team@manulife.com
    """
    )
